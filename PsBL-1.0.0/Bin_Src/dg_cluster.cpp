        ...
        for(auto iter=dg_array.cbegin(); iter!=dg_array.cend(); iter++)
        {
            ...
            if((*iter)->support() >= min_support)
            {
                try
                {
                    ...
                    string seq_1 = genome.seq_frag((*iter)->chr_id_1(), (*iter)->start_1-1, len_1, (*iter)->strand_1()=='+' ? Fasta::POSITIVE : Fasta::NEGATIVE);
                    uLONG len_2 = (*iter)->end_2 - (*iter)->start_2 + 1;
                    string seq_2 = genome.seq_frag((*iter)->chr_id_2(), (*iter)->start_2-1, len_2, (*iter)->strand_2()=='+' ? Fasta::POSITIVE : Fasta::NEGATIVE);
