...
void faformat(const Param &param)
{
    ...
    StringArray chr_ids = fasta.keys();
    ...
    if(not param.rev_anno_pattern.empty())
    {
        ...
        for(auto iter=chr_ids_list.begin(); iter!=chr_ids_list.end(); )
        {
            if( regex_search(fasta.annotation(*iter), reg) )
            {
                iter = chr_ids_list.erase( iter );
            }else{
                iter++;
    ...
    if(not param.fet_anno_pattern.empty())
    {
        ...
        for(auto iter=chr_ids_list.begin(); iter!=chr_ids_list.end(); )
        {
            if( not regex_search(fasta.annotation(*iter), reg) )
            {
                iter = chr_ids_list.erase( iter );
            }else{
                iter++;
    ...
    }else if(param.sort_method == Param::LENGTH)
    {
        ...
        for(const string &chr_id: chr_ids)
            chr_lens.push_back( make_pair( chr_id, fasta.len(chr_id) ) );
    ...
    if(param.write_sto)
    {
        ...
        for(auto iter=chr_ids.cbegin(); iter!=chr_ids.cend(); iter++)
        {
            const string &anno = fasta.annotation(*iter);
            if(not anno.empty())
            {
                has_out = true;
    ...
    for(auto iter=chr_ids.cbegin(); iter!=chr_ids.cend(); iter++)
    {
        if(param.write_sto)
        {
            OUT << setw(longest_name+5) << std::left << *iter << setw(0) << fasta.seq(*iter) << "\n";
        }else{
            OUT << ">" << *iter;
            const string &anno = fasta.annotation(*iter);
            if(not anno.empty())
                OUT << "\t" << anno;
            OUT << "\n" << cut_seq( fasta.seq(*iter), param.number_each_line );
        }
    }
