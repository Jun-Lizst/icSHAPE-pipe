...
def init():
    ...
    if ('input' not in params) or ('output' not in params) or ('columns' not in params):
        print >>sys.stderr, "Error: Please specify -i -o --col"
        print >>sys.stdout, Usage
        sys.exit(-1)
    if ('annotation' not in params) and ('sizeFile' not in params):
        print >>sys.stderr, "Error: Please specify -g or -s"
        print >>sys.stdout, Usage
        sys.exit(-1)
    if ('annotation' in params) and ('sizeFile' in params):
        print >>sys.stderr, "Error: -s and -g cannot be specified simultaneously"
        print >>sys.stdout, Usage
        sys.exit(-1)
    return params
