...
def init():
    params = { 'minBD': 200, 'minAveRT': 2.0, 'minCovNum': 10, 'minCovRatio': 0.1, 'minFPKM':5.0, 'threads': 5 }
    opts, args = getopt.getopt(sys.argv[1:], 'hi:g:s:p:c:T:M:n:m:r:o:')
    ...
    if ('input' not in params) or ('output' not in params):
        print >>sys.stderr, "Error: Please specify -i -o"
        print >>sys.stdout, Usage
        sys.exit(-1)
    ...
    if ('annotation' in params) and ('sizeFile' in params):
        print >>sys.stderr, "Warning: -s is specified, -g is ignored"
...
def read_gTab_head(IN):
    ...
    while line:
        ...
        if tag == 'ColNum':
            ...
            print >>sys.stderr, "Unknown head tag: "+line.strip()
...
def genomeSHAPE2TransSHAPE(chr_id, strand, genomeSHAPEList, Parser, outFile):
    ...
    print >>OUT, "#%s\t%s\t%s\t%s" % (chr_id+strand, exon, intergenic, intron)
...
def saveTransSHAPE(transSHAPE, outFile, FPKMDict=None):
    OUT = open(outFile, 'w')
    for tid in transSHAPE:
        ...
        print >>OUT, "%s\t%s\t%s\t%s" % (tid, length, fpkm, "\t".join(transSHAPE[tid]))
...
def loadChrStatistics(inFile):
    ...
    if not first_line.startswith('#'):
        print >>sys.stderr, "Warning: "+inFile+" doesn't statistics"
...
def parallel_genomeSHAPE2TransSHAPE(inFile, Parser, minBD=100, threads=5, minCovRatio=0.1, minCovNum=10, minAveRT=2, minFPKM=5, FPKMDict=None):
    ...
    while len(process_list) < threads:
        ...
        print >>sys.stdout, "process chr "+chr_id
...
def genomeSHAPEToGenomeSHAPE(genomeSHAPEList, ChrSize):
    genomeSHAPE = {}
    for cur_chr in genomeSHAPEList:
        chr_id, strand = cur_chr[:-1], cur_chr[-1]
        if strand == '-':
            continue
        genomeSHAPE[chr_id] = [('NULL', 'NULL')] * ChrSize[chr_id]
        for pos, shape, RT in genomeSHAPEList[cur_chr]:
            genomeSHAPE[chr_id][pos-1] = (shape,RT)
    return genomeSHAPE
...
def main():
    ...
    if 'fpkmFile' in params:
        print >>sys.stdout, "Start to load fpkm file..."
        fpkm = batch_loadFPKM(params['fpkmFile'].split(','))
    
    if 'annotation' in params:
        print >>sys.stdout, "Start to load annotation file..."
        GAPer = GAP.init(params['annotation'])
        ...
        saveTransSHAPE(transSHAPE, params['output'], FPKMDict=fpkm)
