...
def init():
    ...
    if 'barPos' not in Params:
        print >>sys.stderr, "Error: specify -p"
        print Usage
        exit(-1)
    if 'barLen' not in Params:
        print >>sys.stderr, "Error: specify -l"
        print Usage
        exit(-1)
...
def main():
    ...
    print >>sys.stderr, "split fastq files %s ...\n\t" % (Params['inFile'], )
    ...
    print >>sys.stderr, "File %s successfully splited into:" % (Params['inFile'], )
    for bc in lib_outFile:
        if os.path.exists(lib_outFile[bc]) and lib_outFile[bc] != "":
            print >>sys.stderr, " "+lib_outFile[bc]
    
    print >>sys.stderr, "Splitting finished.\n"
...
def prepareLibraryOutputFiles ( outDir, fileName, fileSuffix, lib_outFile, lib_info, library, mode, compress ):
    ...
    if mode == "new" and os.path.exists(lib_outFile['unmatched']):
        print >>sys.stderr, "Warning! %s exists...will be erased\n" % (lib_outFile['unmatched'], )
        print >>sys.stderr, commands.getoutput("rm "+lib_outFile['unmatched'])
    ...
    for barcode in lib_info:
        ...
        if mode == 'new':
            ...
            if mode == 'new' and os.path.exists(outFile):
                print >>sys.stderr, "Warning! %s exists...will be erased\n" % (outFile, )
                print >>sys.stderr, commands.getoutput("rm "+outFile)
...
def outputStat(statFile, bc_count, lib_info, inFile, library=None, mode='new'):
    if mode == 'new':
        if os.path.exists(statFile):
            print >>sys.stderr, "Warning! %s exists and will be overwitten.\n" % (statFile, )
            print >>sys.stderr, commands.getoutput("rm "+statFile)
