...
def init():
    ...
    if (not params['dotFile']):
        print >>sys.stderr, "Error: Please specify -d"
        print >>sys.stdout, Usage
        sys.exit(-1)
    if (not params['DMSOFiles']) or (not params['NAIFiles']):
        print >>sys.stderr, "Error: Please specify -D and -N"
        print >>sys.stdout, Usage
        sys.exit(-1)
...
def main():
    ...
    print "Will train with window size: ", wSizes
    print "Will train with sunstract factor: ", subFacs
    ...
    for wSize in wSizes:
        for subFac in subFacs:
            ...
            if len(commonTrans) == 0:
                print >>sys.stderr, "Error: no common transcript in SHAPE and dot"
                exit(-1)
    ...
    sns.heatmap(data=record, annot=True, cmap=sns.color_palette("coolwarm", 50))
