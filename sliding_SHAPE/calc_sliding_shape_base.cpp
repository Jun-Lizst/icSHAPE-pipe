...
void print_usage_smartSHAPE()
{
    ...
    const char *help_info = 
            ...
            "\t-enm: <rt/div> enrich method. RT means use raw RT to calculate SHAPE; div means use RT/BD to calculate SHAPE (default: rt)\n"
            ...
            "For DMS-Seq\n"
            "\t-genome: provide a genome file in fasta format (default: None)\n"
            "\t-bases: specify the calculation is constrainted on which bases, such as AC in DMS-Seq (default: None)\n\n"
...
void print_usage_icSHAPE()
{
    ...
    const char *help_info = 
            ...
            "\t-wsize: window size (default: 200)\n"
            "\t-wstep: window step (default: 5)\n"
            "\t-enm: <sub/div/complex> enrich method.(default: complex)\n" 
...
struct General_Param
{
    ...
    /* use only some bases to calculate */

    vector<char> limited_bases;
    string genome_file;
    ...
    operator bool()
    {
        ...
        if((not limited_bases.empty()) and genome_file.empty())
        {
            cerr << RED << "FATAL Error: genome file should be provided when constraint calculation on bases" << DEF << endl;
            return false;
        }
        ...
        if( argv[i][0] == '-' )
        {
            ...
            }else if(not strcmp(argv[i]+1, "bases"))
            {
                ...
                for(int j=0;argv[i+1][j]!='\0';j++)
                    param.bases.push_back(argv[i+1][j])
                i++;
            }
...
void record_param(const General_Param &param, const int argc, char *argv[])
{
    ...
    if(param.mode == TrtCont)
    {
        ...
        if(not param.limited_bases.empty())
        {
            OUT << "Calculation is limited on ";
            for(char base: limited_bases)
                OUT << base << ";";
            OUT << "\n";
            OUT << "genome file: " << param.genome_file << "\n\n";
        }
...
int main(int argc, char *argv[])
{
    ...
    Fasta *p_genome = nullptr;
    if(not param.limited_bases.empty())
    {
        clog << "Bases constrainted, load genome now..." << endl;
        p_genome = new Fasta(param.genome_file);
    }

    ofstream OUT(param.out_file, ofstream::out);
    ...
    if(param.mode == TrtCont)
    {
        ...
        while(sync_chrs(i_vec, chr_ids, record_array))
        {
            ...
            if(strand == '+')
            {
                ...
                sliding_non_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score, chr, POS, OUT, shape_param);

                clog << "Start to sliding_junction..." << endl;
                sliding_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score, chr, POS, OUT, shape_param);
            ...
            else
            {
                ...
                clog << "Start to sliding_non_junction..." << endl;
                sliding_non_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score, chr, NEG, OUT, shape_param);

                clog << "Start to sliding_junction..." << endl;
                sliding_junction( n_rt, d_rt, n_bd, d_bd, junctions[chr_id], score, chr, NEG, OUT, shape_param);
        ...
        while(sync_chrs(i_vec, chr_ids, record_array))
        {
            ...
            if(strand == '+')
            {
                ...
                clog << "Start to sliding_non_junction..." << endl;
                sliding_non_junction( n_rt, n_bd, junctions[chr_id], score, chr, POS, OUT, shape_param);

                clog << "Start to sliding_junction..." << endl;
                sliding_junction( n_rt, n_bd, junctions[chr_id], score, chr, POS, OUT, shape_param);
            }
            else
            {
                ...
                clog << "Start to sliding_non_junction..." << endl;
                sliding_non_junction( n_rt, n_bd, junctions[chr_id], score, chr, NEG, OUT, shape_param);

                clog << "Start to sliding_junction..." << endl;
                sliding_junction( n_rt, n_bd, junctions[chr_id], score, chr, NEG, OUT, shape_param);
            }
