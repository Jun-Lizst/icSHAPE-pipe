...
{
    const char strand_char = (strand == POS) ? '+' : '-';
    ...
    while(cur_i<=chr_size)
    {
        for(long i=0; i<wstep and cur_i<=chr_size; i++)
        {
            ...
            if(NAI_RT[index] >= 1 or DMSO_RT[index] >= 1 or DMSO_BD[index] >= out_min_cov)
            {
                OUT << chr_id << '\t' << strand_char << '\t' << index << '\t'
    ...
    // front and end
/*
    for(long i=0; i<30 and i<=chr_size; i++)
        score[i] = null;
    long s = long(chr_size)-30;
    if(s < 0) s = 0;
    for(long i=s; i<=chr_size; i++)
        score[i] = null;
*/
                        ...
                        ofstream &OUT, const icSHAPE_Param &param)
{
    ...
    for(const Junction &junc: junctions)
    {
        ...
        sliding_single_junction(NAI_RT, DMSO_RT, NAI_BD, DMSO_BD, junc.first, junc.second, score, chr_id, strand, OUT, param);
    }
}
                            ...
                            ofstream &OUT, const icSHAPE_Param &param)
{
    ...
    //uLONG cur_i = start;
    // move to start
    //while(NAI_RT[cur_i] == 0 and cur_i<=end)
    //    ++cur_i;
    ...
    //cout << "index_array size: " << index_array.size() << endl;
    //for(uLONG v: index_array)
     //   cout << v << "\t";
    //cout << "\n";
    ...
    //clog << "Tail and head" << endl;
    // intron tail end head
/*
    for(uLONG i=start; i<start+10 and i<=end; i++)
        score[i] = null;
    uLONG s = start;
    if(end > 10)
        s = end - 10;
    if(s < start) s = start;
    for(uLONG i=s; i<=end; i++)
        score[i] = null;
*/
}
                            ...
                            ofstream &OUT, const smartSHAPE_Param &param)
{
    ...
    //cout << param.out_min_cov << "\t" << param.min_cov << "\t" << param.wsize << "\t" << param.wstep << endl;
    ...
    while(cur_i<=chr_size)
    {
        for(long i=0; i<wstep and cur_i<=chr_size; i++)
        {
            ...
            if(NAI_RT[index] >= 1 or NAI_BD[index] >= out_min_cov)
            {
                OUT << chr_id << '\t' << strand_char << '\t' << index << '\t'
                    << NAI_RT[index] << '\t' << NAI_BD[index] << '\t'
                    << shape_score << '\t' << valid_count << '\t';
                        ...
                        ofstream &OUT, const smartSHAPE_Param &param )
{
    ...
    for(const Junction &junc: junctions)
    {
        if(++index % 100 == 0)
            clog << "\t junc ratio: " << 100.0*index/junc_num << "%" << endl;
        sliding_single_junction(NAI_RT, NAI_BD, junc.first, junc.second, score, chr_id, strand, OUT, param);
    }
}
...
/*
    if(precalculated.size() != index_array.size())
    {
        cerr << RED << "FATAL Error: different size in 1" << endl;
        exit(-1); 
    }
...
/*
    if(precalculated.size() != index_array.size())
    {
        cerr << RED << "FATAL Error: different size in 3" << endl;
        exit(-1); 
    }
*/
...
/*
    for(long i=start; i<start+10 and i<=end; i++)
        score[i] = null;
    long s = long(end)-10;
    if(s < start) s = start;
